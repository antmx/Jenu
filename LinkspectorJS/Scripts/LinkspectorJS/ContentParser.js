...
/// <reference path="~/Scripts/LinkspectorJS/Resource.js" />
/// <reference path="~/Scripts/LinkspectorJS/Url.js" />

// Set up LinkspectorJS.Core namespace
Namespace.Create("LinkspectorJS.Core");

// Define LinkspectorJS.Core.ContentParser type
LinkspectorJS.Core.ContentParser = function () {
	this._content = null;
...
LinkspectorJS.Core.ContentParser.prototype = {
	...
	ParseCssImports: function (content) {
		...
		do {
			...
			if (match) {
				...
				var objUrl = new LinkspectorJS.Core.Url(url, this._contentUrl.href);
				urls.push(objUrl);
			}
		} while (match);
